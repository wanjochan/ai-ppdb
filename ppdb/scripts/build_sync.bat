...
rem Build sync test
echo Building sync %SYNC_MODE% test...
"%GCC%" %CFLAGS% ^
    "%PPDB_DIR%\src\base.c" ^
    "%PPDB_DIR%\test\white\test_framework.c" ^
    "%PPDB_DIR%\test\white\infra\test_sync.c" ^
    %LDFLAGS% %LIBS% -o "%BUILD_DIR%\sync_%SYNC_MODE%_test.exe.dbg"
if errorlevel 1 exit /b 1

"%OBJCOPY%" -S -O binary "%BUILD_DIR%\sync_%SYNC_MODE%_test.exe.dbg" "%BUILD_DIR%\sync_%SYNC_MODE%_test.exe"
if errorlevel 1 exit /b 1

rem Run the test if not explicitly disabled
if not "%3"=="norun" (
    "%BUILD_DIR%\sync_%SYNC_MODE%_test.exe"
)
