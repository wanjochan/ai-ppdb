...
set "NEED_REBUILD=0"
if not exist "%BUILD_DIR%\cosmopolitan.a" set "NEED_REBUILD=1"
if not exist "%BUILD_DIR%\ape.lds" set "NEED_REBUILD=1"
if not exist "%BUILD_DIR%\crt.o" set "NEED_REBUILD=1"
if not exist "%BUILD_DIR%\ape.o" set "NEED_REBUILD=1"
if not exist "%BUILD_DIR%\cosmopolitan.h" set "NEED_REBUILD=1"
...
rem Check directories only if we need to rebuild
if "%NEED_REBUILD%"=="1" (
    if not exist "%COSMO%" (
...
if "%RELEASE_FLAGS%"=="" set "RELEASE_FLAGS=-O3 -DNDEBUG"

rem Build Cosmopolitan objects only if needed
if "%NEED_REBUILD%"=="1" (
    ...
    echo Cosmopolitan build is up to date
)
