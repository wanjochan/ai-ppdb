...
rem 设置编译选项
set COSMOPUB=D:\dev\ai-ppdb\repos\cosmopolitan_pub
...
echo Linking test_target.exe.dbg...
%LD% -T %COSMOPUB%\ape.lds --gc-sections --build-id=none -z max-page-size=4096 --omagic test_target.o %COSMOPUB%\ape.o %COSMOPUB%\crt.o %COSMOPUB%\cosmopolitan.a -o test_target.exe.dbg
if errorlevel 1 goto error

echo Creating binary test_target.exe...
%OBJCOPY% -S -O binary test_target.exe.dbg test_target.exe
if errorlevel 1 goto error

echo Checking file sizes...
dir test_target.o test_target.exe.dbg test_target.exe
