...
```c
struct perf_metrics {
    uint64_t start_time;      // 测试开始时�?    uint64_t end_time;        // 测试结束时间
    uint64_t total_ops;       // 总操作数
    uint64_t success_ops;     // 成功操作�?    uint64_t failed_ops;      // 失败操作�?    size_t peak_memory;       // 峰值内�?    size_t current_memory;    // 当前内存
...
### 1. 基准测试（Benchmark�?- 顺序写入测试�?00K 条记录）
  * 验证基础写入性能
  * 观察 WAL 写入效率
  * 监控内存增长情况
