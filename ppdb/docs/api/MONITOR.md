...
typedef struct ppdb_perf_metrics {
    ...
    atomic_uint_least64_t total_latency_us;   // 总延迟（微秒�?    atomic_uint_least64_t max_latency_us;     // 最大延迟（微秒�?    atomic_uint_least64_t lock_contentions;   // 锁竞争次�?    atomic_uint_least64_t lock_wait_us;       // 锁等待时�?} ppdb_perf_metrics_t;
```
...
typedef struct ppdb_monitor {
    ...
    atomic_bool should_switch;                // 是否需要切换标�?    time_t window_start_ms;                   // 当前窗口开始时�?    int cpu_cores;                            // CPU核心�?} ppdb_monitor_t;
```

## 功能接口

### 监控器管�?```c
// 创建监控�?ppdb_monitor_t* ppdb_monitor_create(void);
...
### 性能指标收集
```c
// 记录操作开�?void ppdb_monitor_op_start(ppdb_monitor_t* monitor);

// 记录操作结束
void ppdb_monitor_op_end(ppdb_monitor_t* monitor, uint64_t latency_us);

// 记录锁竞�?void ppdb_monitor_lock_contention(ppdb_monitor_t* monitor, uint64_t wait_us);
```
...
// 获取锁竞争率（百分比�?double ppdb_monitor_get_contention_rate(ppdb_monitor_t* monitor);
```
...
### 基本使用
```c
// 创建监控�?ppdb_monitor_t* monitor = ppdb_monitor_create();
...
// 检查性能状�?if (ppdb_monitor_should_switch(monitor)) {
    // 切换到分片模�?}
