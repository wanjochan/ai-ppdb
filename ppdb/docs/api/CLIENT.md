...
ppdb> help
Available commands:
  auth status            显示当前认证状�?  auth list             列出所有用�?  auth create <user>    创建新用�?  auth grant <priv>     授予权限
  get <key>            获取键�?  set <key> <value>    设置键�?  delete <key>         删除键�?  stats                显示服务器状�?  clients              显示已连接客户端
...
Options:
  ...
  -p, --port <port>     服务器端�?(默认: 11211)
  -u, --user <user>     用户�?  -P, --password        提示输入密码
...
### 特点
- 支持交互式和命令行两种模�?- 批处理支�?- 格式化输�?- 连接池优�?- 错误重试机制

## 3. 认证与权限管�?
### 权限级别
- **admin**: 完全访问权限，包括用户管�?- **write**: 读写数据权限
- **read**: 只读权限
- **stats**: 只能查看统计信息

### 用户管理命令
```bash
# 创建新用�?ppdb> auth create john
New password: ****
Confirm password: ****
User 'john' created.
...
### 安全建议
1. 定期更改密码
2. 使用强密�?3. 根据最小权限原则分配权�?4. 避免在脚本中明文存储密码
...
// 创建客户�?ppdb_client_t* client = ppdb_client_create();
ppdb_client_connect(client, "localhost", 11211);
...
# 同步客户�?client = Client('localhost', 11211)
client.set('key', 'value')
value = client.get('key')

# 异步客户�?async with AsyncClient('localhost', 11211) as client:
