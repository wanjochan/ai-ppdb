...
## 构建脚本 (scripts/)
- `build.bat` - Windows 平台构建脚本�?  * 参数格式：`build.bat [target] [mode]`
...
### 测试入口
- `test_main.c` - 主测试入�?  * 初始化测试环�?  * 运行测试用例
  * 输出测试结果
...
### 2. 命名规范
- 公共接口前缀：`ppdb_`
- 内部接口前缀：根据模块定义（�?`sync_`�?- 测试文件前缀：`test_`

### 3. 头文件包�?- 优先使用项目根目录的相对路径
- 公共头文件使�?`<>` 包含
- 内部头文件使�?`""` 包含

### 4. 注释规范
- 文件头部：文件说明、作者、日�?- 函数：功能说明、参数、返回�?- 复杂逻辑：实现说明、注意事�?- 中文注释：使�?UTF-8 编码 