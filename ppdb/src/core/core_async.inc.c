...
struct ppdb_core_async_loop {
    int epoll_fd;           // epoll 文件描述符
    bool is_running;        // 事件循环是否在运行
    ppdb_core_mutex_t* mutex;  // 保护内部状态
};

struct ppdb_core_async_handle {
    ppdb_core_async_loop_t* loop;  // 所属事件循环
    int fd;                        // 文件描述符
    void* data;                    // 用户数据
    ppdb_core_async_cb callback;   // 回调函数
    struct {
        void* buf;                 // 读写缓冲区
        size_t len;                // 缓冲区长度
        size_t pos;                // 当前位置
...
struct ppdb_core_async_future {
    ppdb_core_async_loop_t* loop;  // 所属事件循环
    bool is_ready;                 // 是否就绪
    void* result;                  // 结果数据
    ppdb_core_mutex_t* mutex;      // 保护内部状态
    ppdb_core_cond_t* cond;        // 条件变量
};
...
ppdb_error_t ppdb_core_async_loop_create(ppdb_core_async_loop_t** loop) {
    ...
    *loop = ppdb_core_alloc(sizeof(ppdb_core_async_loop_t));
    if (!*loop) return PPDB_ERR_OUT_OF_MEMORY;
    
    memset(*loop, 0, sizeof(ppdb_core_async_loop_t));
...
ppdb_error_t ppdb_core_async_loop_run(ppdb_core_async_loop_t* loop, int timeout_ms) {
    ...
    while (1) {
        ...
        for (int i = 0; i < nfds; i++) {
            ppdb_core_async_handle_t* handle = events[i].data.ptr;
...
ppdb_error_t ppdb_core_async_handle_create(ppdb_core_async_loop_t* loop,
    ...
    *handle = ppdb_core_alloc(sizeof(ppdb_core_async_handle_t));
    if (!*handle) return PPDB_ERR_OUT_OF_MEMORY;
    
    memset(*handle, 0, sizeof(ppdb_core_async_handle_t));
    (*handle)->loop = loop;
    (*handle)->fd = fd;
...
ppdb_error_t ppdb_core_async_future_create(ppdb_core_async_loop_t* loop,
    ...
    *future = ppdb_core_alloc(sizeof(ppdb_core_async_future_t));
    if (!*future) return PPDB_ERR_OUT_OF_MEMORY;
    
    memset(*future, 0, sizeof(ppdb_core_async_future_t));
    (*future)->loop = loop;
...
ppdb_error_t ppdb_core_async_future_is_ready(ppdb_core_async_future_t* future,
                                            bool* ready) {
