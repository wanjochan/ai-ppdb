...
typedef struct client_state {
    ...
    ppdb_database_async_handle_t* handle; // Client handle
} client_state_t;
...
struct ppdb_server {
    ...
    ppdb_database_async_loop_t* loop;     // Event loop
    ppdb_database_async_handle_t* listen_handle; // Listen handle
...
static void on_client_data(ppdb_database_async_handle_t* handle, const uint8_t* data, size_t size);
static void on_client_close(ppdb_database_async_handle_t* handle);
static void on_new_connection(ppdb_database_async_handle_t* handle, int client_fd);
...
static void* ppdb_server_alloc(size_t size) {
    void* ptr = ppdb_database_malloc(size);
    if (ptr) {
        memset(ptr, 0, size);
    }
    return ptr;
...
ppdb_error_t ppdb_server_create(ppdb_ctx_t* ctx, const peer_ops_t* ops, struct ppdb_server** server) {
    ...
    *server = ppdb_server_alloc(sizeof(struct ppdb_server));
    ...
    // Create event loop
    ppdb_error_t err = ppdb_database_async_loop_create(&(*server)->loop);
    if (err != PPDB_OK) {
    ...
    err = ppdb_database_async_handle_create((*server)->loop, &(*server)->listen_handle);
    if (err != PPDB_OK) {
        ppdb_database_async_loop_destroy((*server)->loop);
        ppdb_server_free(*server);
        *server = NULL;
        return err;
    }
    ...
    // Set callbacks
    ppdb_database_async_handle_set_connection_cb((*server)->listen_handle, on_new_connection);
    
    // Set user data
    ppdb_database_async_handle_set_data((*server)->listen_handle, *server);
...
ppdb_error_t ppdb_server_start(struct ppdb_server* server) {
    ...
    server->running = true;
    return ppdb_database_async_loop_run(server->loop);
}
...
ppdb_error_t ppdb_server_stop(struct ppdb_server* server) {
    ...
    server->running = false;
    ppdb_database_async_loop_stop(server->loop);
    return PPDB_OK;
}
...
void ppdb_server_destroy(struct ppdb_server* server) {
    ...
    if (server->listen_handle) {
        ppdb_database_async_handle_destroy(server->listen_handle);
    }
...
// Handle client data
static void on_client_data(ppdb_database_async_handle_t* handle, const uint8_t* data, size_t size) {
    client_state_t* state = (client_state_t*)ppdb_database_async_handle_get_data(handle);
    if (!state) {
        return;
    }
    ...
    if (err != PPDB_OK) {
        ...
        ppdb_database_async_handle_close(handle);
        return;
    }
    
    // Set socket
    err = ppdb_conn_set_socket(conn, ppdb_database_async_handle_get_fd(handle));
    if (err != PPDB_OK) {
...
static void on_new_connection(ppdb_database_async_handle_t* handle, int client_fd) {
    struct ppdb_server* server = (struct ppdb_server*)ppdb_database_async_handle_get_data(handle);
    if (!server) {
        close(client_fd);
        return;
    }
    ...
    ppdb_database_async_handle_t* client_handle = NULL;
    ppdb_error_t err = ppdb_database_async_handle_create(server->loop, &client_handle);
    if (!client_handle) {
    ...
    if (!state) {
        ...
        ppdb_database_async_handle_destroy(client_handle);
        close(client_fd);
        return;
    }
    ...
    // Set callbacks
    ppdb_database_async_handle_set_data_cb(client_handle, on_client_data);
    ppdb_database_async_handle_set_close_cb(client_handle, on_client_close);
    
    // Set user data
    ppdb_database_async_handle_set_data(client_handle, state);
    
    // Accept connection
    err = ppdb_database_async_handle_accept(client_handle, client_fd);
    if (err != PPDB_OK) {
