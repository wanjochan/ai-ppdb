...
static void test_skiplist_basic(void) {
    ...
    // Create skip list
    err = ppdb_base_skiplist_create(&list, compare_int);
    assert(err == PPDB_OK);
    assert(list != NULL);

    // Insert some values
    err = ppdb_base_skiplist_insert(list, (void*)1, (void*)"one");
    assert(err == PPDB_OK);
    err = ppdb_base_skiplist_insert(list, (void*)2, (void*)"two");
    assert(err == PPDB_OK);
    err = ppdb_base_skiplist_insert(list, (void*)3, (void*)"three");
    assert(err == PPDB_OK);
    ...
    // Remove values
    err = ppdb_base_skiplist_remove(list, (void*)2);
    assert(err == PPDB_OK);
