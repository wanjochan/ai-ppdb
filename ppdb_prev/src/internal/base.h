...
typedef struct ppdb_error_stats_s {
    ...
    uint64_t error_trend[24];        // 最近24小时错误趋势
    uint64_t trend_start_time;       // 趋势统计开始时间
} ppdb_error_stats_t;
...
// New error statistics functions
ppdb_error_t ppdb_error_stats_update(ppdb_error_t code,
                                    ppdb_error_severity_t severity,
                                    ppdb_error_category_t category);
ppdb_error_t ppdb_error_stats_get(ppdb_error_stats_t* stats);
void ppdb_error_stats_reset(void);
ppdb_error_t ppdb_error_stats_analyze_trend(uint64_t* trend_data, 
                                          size_t* trend_size);

// Error trend analysis
typedef struct ppdb_error_trend_s {
    uint64_t window_start;           // 统计窗口开始时间
    uint64_t window_size;            // 统计窗口大小(秒)
    uint64_t error_count;            // 窗口内错误数
    uint64_t peak_error_rate;        // 峰值错误率
    double avg_error_rate;           // 平均错误率
    ppdb_error_severity_t highest_severity; // 最高错误级别
    ppdb_error_category_t main_category;    // 主要错误类别
} ppdb_error_trend_t;
...
typedef struct ppdb_error_log_config_s {
    const char* log_dir;           // 日志目录
    size_t max_file_size;         // 单个日志文件最大大小
    size_t max_files;             // 最大日志文件数
    bool compress_old;            // 是否压缩旧日志
    bool async_write;             // 是否异步写入
} ppdb_error_log_config_t;

// Error log functions
ppdb_error_t ppdb_base_error_log_init(const ppdb_error_log_config_t* config);
ppdb_error_t ppdb_base_error_log_write(ppdb_error_t code,
                                     ppdb_error_severity_t severity,
                                     ppdb_error_category_t category,
                                     const char* file,
                                     int line,
                                     const char* func,
                                     const char* message);
ppdb_error_t ppdb_base_error_log_rotate(void);
ppdb_error_t ppdb_base_error_log_cleanup(void);
