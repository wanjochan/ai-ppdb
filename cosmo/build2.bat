...
echo Building main2.exe...
"%GCC%" %CFLAGS% -Id:\dev\ai-ppdb\cosmopolitan -include d:\dev\ai-ppdb\cosmopolitan\cosmopolitan.h -static -nostdlib -Wl,-T,d:\dev\ai-ppdb\cosmo\build\ape.lds -Wl,--gc-sections -fuse-ld=bfd -Wl,-z,max-page-size=0x1000 -no-pie -o "%BUILD_DIR%\main2.dbg" main2.c "%BUILD_DIR%\crt.o" "%BUILD_DIR%\ape.o" "%BUILD_DIR%\cosmopolitan.a"
if errorlevel 1 goto :error

echo Converting to APE format...
"%OBJCOPY%" -O binary "%BUILD_DIR%\main2.dbg" main2.exe
if errorlevel 1 goto :error
